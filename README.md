...
| 页面 | 说明 |
| --- | --- |
| **字体管理** | 树形浏览字体库文件夹，两列展示（文件名 + Windows 标准字体名，简→繁→日→英优先取用），勾选即注册到 Windows（当前会话有效），系统已装字体灰显标记；目录级三态勾选、按名称/家族名筛选、底部多语言预览；**字幕适配**：把 `.ass`/`.ssa` 字幕用到的字体名批量替换为字体库字体 |
...
| **设置** | 主题模式、预览文字与字号、开机自动启动、自动恢复选中、关于与检查更新 |
...
5. 在**设置**页配置主题、预览文字与字号、开机自启与自动恢复选中。
