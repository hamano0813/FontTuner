...
拾字 FontTuner 是一个基于 PySide6 + [PySide6-Fluent-Widgets](https://github.com/zhiyiYo/PySide6-Fluent-Widgets)（qfw）的 Windows 桌面应用。它围绕字体文件（`.ttf` / `.otf` / `.ttc` / `.otc`）的 **name 表元数据** 提供批量编辑能力，并附带解包打包与字体注册管理。
...
- Windows 10 / 11（64 位）
- Python 3.14+
- [uv](https://docs.astral.sh/uv/)（开发与发布构建工具链）
...
> 数据文件 `data/templates.json`、`data/translations.json` 与 `config.json` 在开发态存放于仓库根；打包安装后自动改存 `%APPDATA%\FontTuner`（安装根不可写时回落），首次运行按默认值自动生成。

## 发布构建

一键脚本仿 srw_alpha 的发布方案，走 **Inno Setup 安装器 + 在线部署**（安装时经 uv 下载 Python 并同步依赖，安装包小、无需预装 Python）：
...
构建产物（`dist/`）：
- `FontTuner-<版本>-x64.zip` — 内含 Inno Setup 安装器 `FontTuner-<版本>-x64.exe`。
